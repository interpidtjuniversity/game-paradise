import {
	...
	BULLET_TYPE_PLAYER,
	DOWN, ENEMY_LOCATION,
	GAME_STATE_INIT, GAME_STATE_MENU, GAME_STATE_OVER,
	GAME_STATE_START,
	GAME_STATE_WIN,
	LEFT, POS, RESOURCE_IMAGE,
	RIGHT,
	SCREEN_HEIGHT,
	SCREEN_WIDTH,
	UP, WALL
...
import {Map} from "./map";
import {EnemyOne, EnemyThree, EnemyTwo, PlayTank} from "./tank";
...
function initScreen(){
	...
	wallCtx = document.getElementById("wallCanvas").getContext("2d");
	//grassCtx = $("#grassCanvas")[0].getContext("2d");
	grassCtx = document.getElementById("grassCanvas").getContext("2d");
	//$("#wallCanvas").attr({"width":SCREEN_WIDTH});
	document.getElementById("wallCanvas").setAttribute("width",SCREEN_WIDTH);
	//$("#wallCanvas").attr({"height":SCREEN_HEIGHT});
	document.getElementById("wallCanvas").setAttribute("height",SCREEN_HEIGHT);
	//$("#grassCanvas").attr({"width":SCREEN_WIDTH});
	document.getElementById("grassCanvas").setAttribute("width",SCREEN_WIDTH);
	//$("#grassCanvas").attr({"height":SCREEN_HEIGHT});
	document.getElementById("grassCanvas").setAttribute("height",SCREEN_HEIGHT);
	//tankCtx = $("#tankCanvas")[0].getContext("2d");
	tankCtx = document.getElementById("tankCanvas").getContext("2d");
	//$("#tankCanvas").attr({"width":SCREEN_WIDTH});
	document.getElementById("tankCanvas").setAttribute("width",SCREEN_WIDTH);
	//$("#tankCanvas").attr({"height":SCREEN_HEIGHT});
	document.getElementById("tankCanvas").setAttribute("height",SCREEN_HEIGHT);
	//overCtx = $("#overCanvas")[0].getContext("2d");
	overCtx = document.getElementById("overCanvas").getContext("2d");
	//$("#overCanvas").attr({"width":SCREEN_WIDTH});
	document.getElementById("overCanvas").setAttribute("width",SCREEN_WIDTH);
	//$("#overCanvas").attr({"height":SCREEN_HEIGHT});
	document.getElementById("overCanvas").setAttribute("height",SCREEN_HEIGHT);
	$("#canvasDiv").css({"width":SCREEN_WIDTH});
	$("#canvasDiv").css({"height":SCREEN_HEIGHT});
	$("#canvasDiv").css({"background-color":"#000000"});
	//$("#canvasDiv").css({"background-color":"red"});
